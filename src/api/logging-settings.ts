...
function ensureLoggingSettings(guildId: string): LoggingSettings | null {
  try {
    ...
    const stmt = db.prepare(`
      INSERT INTO logging_settings (guild_id, message_delete_logging, message_edit_logging, command_logging)
      VALUES (?, 1, 1, 1)
    `);
    stmt.run(guildId);
...
// POST /api/servers/:serverId/logging-settings (same functionality as PUT)
router.post('/:serverId/logging-settings',
  createValidationMiddleware({ serverId: commonSchemas.discordId }, 'params'),
  async (req: Request, res: Response) => {
    try {
      const { serverId } = req.params;
      ...
      logInfo('Logging Settings API', `Creating/updating logging settings for server ${serverId}: ${JSON.stringify(updates)}`);
      ...
      if (!success) {
        logError('Logging Settings API', `Failed to create/update settings for server ${serverId}`);
        return res.status(500).json({
          success: false,
          error: 'Failed to create/update logging settings'
        });
      }
      ...
      logInfo('Logging Settings API', `Successfully created/updated settings for server ${serverId}`);
      ...
      try {
        ...
        await DashboardLogsService.logActivity({
          ...
          target_type: 'server',
          target_id: serverId,
          old_value: 'Previous logging settings',
          new_value: JSON.stringify(responseSettings),
          details: `⚙️ Logging Settings Update: ${adminUsername} updated logging settings for server ${serverId}.\n📝 Changes: Message Delete=${responseSettings.message_delete_logging ? 'ENABLED' : 'DISABLED'} | Message Edit=${responseSettings.message_edit_logging ? 'ENABLED' : 'DISABLED'} | Commands=${responseSettings.command_logging ? 'ENABLED' : 'DISABLED'}`,
          success: true,
          guild_id: serverId
        });
      } catch (logErr) {
      ...
      logError('Logging Settings API', `Error creating/updating settings for server ${req.params.serverId}: ${error.message}`);
      return res.status(500).json({
        success: false,
        error: 'Internal server error'
      });
