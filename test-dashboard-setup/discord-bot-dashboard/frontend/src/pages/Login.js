...
export default function Login() {
  const handleLogin = () => {
    // In production, redirect to the Discord OAuth endpoint
    // In development, this will be handled by the mock authentication
    if (process.env.NODE_ENV === 'production') {
      window.location.href = '/api/auth/discord';
    } else {
      // For development, just redirect to the home page
      // The ProtectedRoute component in App.js will handle setting a mock user
      ...
      <LoginCard>
        ...
        <CardContent sx={{ textAlign: 'center', py: 4 }}>
          ...
          <Typography variant="body1" color="text.secondary" paragraph sx={{ mb: 4 }}>
            Manage your Discord bot's settings, tickets, verification, and more all in one place.
          </Typography>
          <DiscordButton 
            variant="contained" 
            size="large"
